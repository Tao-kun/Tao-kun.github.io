...
若$\bar{\boldsymbol{x}} \in C$，由于$\boldsymbol{x} \in ri(C)$，故存在开球$S = \{\boldsymbol{z} \ \vert \ \|\boldsymbol{z} - \boldsymbol{x}\| < \varepsilon\}$使得$S \cap aff(C) \subseteq C$.对于任意$\alpha \in (0,1]$，设$\boldsymbol{x}_\alpha = \alpha \boldsymbol{x} + (1 - \alpha) \bar{\boldsymbol{x}}$且$S_\alpha = \{\boldsymbol{z} \ \vert \ \|\boldsymbol{z} - \boldsymbol{x}_\alpha\| < \alpha \varepsilon\}$，于是$S_\alpha \cap aff(C)$中的任意一点都可以看成$S \cap aff(C)$中某点与$\bar{\boldsymbol{x}}$的凸组合，由$C$的凸性知$S_\alpha \cap aff(C) \subseteq C$，故$\boldsymbol{x}_\alpha \in ri(C)$.

若$\bar{\boldsymbol{x}} \not \in C$，由于$\boldsymbol{x} \in ri(C)$，故存在开球$S = \{\boldsymbol{z} \ \vert \ \|\boldsymbol{z} - \boldsymbol{x}\| < \varepsilon\}$使得$S \cap aff(C) \subseteq C$.对于任意$\alpha \in (0,1]$，设$\boldsymbol{x}_\alpha = \alpha \boldsymbol{x} + (1 - \alpha) \bar{\boldsymbol{x}}$，下面证明$\boldsymbol{x}_\alpha \in ri(C)$.考虑收敛于$\bar{\boldsymbol{x}}$的序列$\{\boldsymbol{x}_k\} \subseteq C$，设$\boldsymbol{x}_{k,\alpha} = \alpha \boldsymbol{x} + (1 - \alpha) \boldsymbol{x}_k$，显然$\{ \boldsymbol{z} \ \vert \ \|\boldsymbol{z} - \boldsymbol{x}_{k,\alpha} \| < \alpha \varepsilon\} \cap aff(C) \subseteq C$. 又$\boldsymbol{x}_{k,\alpha} \rightarrow \boldsymbol{x}_\alpha$，故当$k$足够大时有$$\begin{align*} \{ \boldsymbol{z} \ \vert \ \|\boldsymbol{z} - \boldsymbol{x}_\alpha \| < \alpha \varepsilon / 2\} \subseteq \{ \boldsymbol{z} \ \vert \ \|\boldsymbol{z} - \boldsymbol{x}_{k,\alpha} \| < \alpha \varepsilon \} \end{align*}$$这意味着$\{ \boldsymbol{z} \ \vert \ \|\boldsymbol{z} - \boldsymbol{x}_\alpha \| < \alpha \varepsilon / 2\} \cap aff(C) \subseteq C$，故$\boldsymbol{x}_\alpha \in ri(C)$.
