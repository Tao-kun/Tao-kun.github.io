...
由(a)知$$\begin{align*} \boldsymbol{\mu}^\top \boldsymbol{u} + \beta w \geq \gamma,  \forall (\boldsymbol{u}, w) \in cl(C), \end{align*}$$将上式乘以$\varepsilon > 0$加上式(\ref{equ: nonvertical hyperplane})可得$$\begin{align*} (\bar{\boldsymbol{\mu}} + \varepsilon \boldsymbol{\mu})^\top \boldsymbol{u} + \varepsilon \beta w > \bar{\gamma} + \varepsilon \gamma, \forall (\boldsymbol{u}, w) \in cl(C), \forall \varepsilon > 0. \end{align*}$$又$\bar{\gamma} > \bar{\boldsymbol{\mu}}^\top \bar{\boldsymbol{u}}$，故对于充分小的$\varepsilon$有$$\begin{align*} \bar{\gamma} + \varepsilon \gamma > (\bar{\boldsymbol{\mu}} + \varepsilon \boldsymbol{\mu})^\top \bar{\boldsymbol{u}} + \varepsilon \beta \bar{w}. \end{align*}$$结合上面两式可得$$\begin{align*} (\bar{\boldsymbol{\mu}} + \varepsilon \mu)^\top \boldsymbol{u} + \varepsilon \beta w > (\bar{\boldsymbol{\mu}} + \varepsilon \mu)^\top \bar{\boldsymbol{u}} + \varepsilon \beta \bar{w},  \forall (\boldsymbol{u}, w) \in cl(C), \end{align*}$$故存在非垂直超平面严格分离$(\bar{\boldsymbol{u}}, \bar{w})$和$cl(C)$，其对应的法向量为$(\bar{\boldsymbol{\mu}} + \varepsilon \mu, \varepsilon \beta)$，又$C \subseteq  cl(C)$，故该超平面严格分离$(\bar{\boldsymbol{u}}, \bar{w})$和$C$.
